...
- Need to register a "Desktop app" OAuth client on a w159 GCP project
...
2. `hermes_cli/models.py` — Add Gemini model catalog
3. `hermes_cli/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
4. `hermes_cli/main.py` — Add `_model_flow_gemini()`, add to provider choices
5. `hermes_cli/setup.py` — Add gemini auth flow (trigger browser OAuth)
6. `run_agent.py` — Token refresh before API calls (like Copilot pattern)
7. `agent/auxiliary_client.py` — Add gemini to aux resolution chain
8. `agent/model_metadata.py` — Add Gemini model context lengths

### Tests

10. `tests/agent/test_google_oauth.py` — OAuth flow unit tests
2. `tests/test_api_key_providers.py` — Add gemini provider test

### Docs

12. `website/docs/getting-started/quickstart.md` — Add gemini to provider table
2. `website/docs/user-guide/configuration.md` — Gemini setup section
3. `website/docs/reference/environment-variables.md` — New env vars
...
- w159 GCP project with Desktop OAuth client registered
