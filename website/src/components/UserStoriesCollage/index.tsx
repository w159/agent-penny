    ...
    const visible = useMemo(() => {
        return allStories.filter((s) => {
            if (activeCategory !== "all" && s.category !== activeCategory)
                return false;
            if (activeSource !== "all" && s.source !== activeSource)
                return false;
            return true;
        });
    ...
    return (
        <div className={styles.wrap}>
            <div className={styles.hero}>
                ...
                <p>
                    What the Hermes Agent community is actually building. Every
                    tile below links to a real post, issue, video, or gist where
                    someone describes how they use Hermes &mdash; scraped from
                    X, GitHub, Reddit, Hacker News, YouTube, blogs, and
                    podcasts.
                </p>
                <div className={styles.meta}>
                    ...
                    <span>
                        <strong>{Object.keys(sourceCounts).length}</strong>{" "}
                        sources
                    </span>
                </div>
            </div>
                    ...
                    .map(([key, label]) => (
                        <button
                            ...
                            style={{
                                ...
                                ...(activeSource === key ?
                                    {
                                        background: sourceColor(key),
                                        borderColor: sourceColor(key),
                                        color: "#fff",
                                    }
                                :   {}),
                            }}
            ...
            :   <div className={styles.grid}>
                    {visible.map((s) => {
                        const cat =
                            CATEGORIES[s.category] ?? CATEGORIES.general;
                        const sizeClass =
                            s.size === "lg" ? styles.tileLg
                            : s.size === "sm" ? styles.tileSm
                            : styles.tileMd;
            ...
            <div className={styles.footer}>
                ...
                <a
                    href="https://github.com/w159/agent-penny/edit/main/website/src/data/userStories.json"
                    target="_blank"
                    rel="noopener noreferrer"
                >
