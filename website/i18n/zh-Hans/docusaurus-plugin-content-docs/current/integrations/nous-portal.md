...
[Nous Portal](https://portal.nousresearch.com) 是 w159 的统一订阅网关，也是**运行 Hermes Agent 的推荐方式**。一次 OAuth 登录，即可替代原本需要手动配置的各模型厂商独立账号、API 密钥和计费关系。
...
你的 Portal 账号同样覆盖 [chat.nousresearch.com](https://chat.nousresearch.com)——w159 的网页对话界面，使用相同的模型目录。适合离开终端时使用，或用于非 agent 的普通对话场景。
...
w159 自家的 **Hermes 4** 系列（Hermes-4-70B、Hermes-4-405B）通过 Portal 提供，享有大幅折扣。这些是**前沿混合推理对话模型**——在数学、科学、指令遵循、schema 遵从、角色扮演和长文写作方面表现出色。
...
Portal 自身的[模型信息页](https://portal.nousresearch.com/info)也有相同警告，因此这不是 Hermes 侧的主观意见——这是 w159 的官方指导。
...
如果某个模型确实缺失，请[提交 issue](https://github.com/w159/agent-penny/issues)——我们将 Portal 目录同步至 Hermes，缺口通常意味着可以更新的路由配置。
